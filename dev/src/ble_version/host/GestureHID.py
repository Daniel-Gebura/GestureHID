...
# NEW: BLE output backend (adapter that mirrors HIDController API)
from BLEHIDController import BLEHIDController
...
# BLE HID Proxy Settings (must match your nRF code)
BLE_DEVICE_NAME = "HID-Proxy-Control"
BLE_AUTH_TOKEN = "CHANGE_ME"
...
def main():
    ...
    # Initialize HID output backend (BLE -> nRF -> USB HID to target PC)
    hid = BLEHIDController(device_name=BLE_DEVICE_NAME, auth_token=BLE_AUTH_TOKEN)
    hid.connect()

    # Initialize HID FSMs
    toggle_fsm = HIDToggleFSM()  # State machine to toggle HID control on/off
    mouse_fsm = MouseFSM(hid=hid, sensitivity=MOUSE_SENSITIVITY)  # State machine to control mouse
    keyboard_fsm = KeyboardFSM(hid=hid)  # State machine to control keyboard press/release
    ...
    try:
        ...
        while cap.isOpened():
            ...
            if toggle_fsm.update(right_gesture):
                ...
                # Safety: when disabling HID, force release states
                if not hid_enabled:
                    hid.release_keys()
                    hid.release_mouse()
