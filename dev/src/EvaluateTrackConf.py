...
def run_experiments():
    ...
    for conf in TRACKING_CONFIDENCES:
        results.append(evaluate(VIDEO_PATH, LEFT_LABEL_CSV, RIGHT_LABEL_CSV,
                                ...
                                16, conf, GESTURE_CONFIDENCE, "tracking_conf", conf))
    ...
    # Save data
    df.to_csv(os.path.join(RESULTS_DIR, "track_conf_full_results.csv"), index=False)
    acc_df = pd.DataFrame(class_acc)
    acc_df.to_csv(os.path.join(RESULTS_DIR, "track_conf_per_class_accuracy.csv"), index=False)


    # Insert plots here
    sns.set(style="whitegrid")
    
    # === PLOT 1: Latency Breakdown per Tracking Confidence ===
    plt.figure(figsize=(10, 6))
    latency_df = pd.DataFrame({
        "Tracking Confidence": df["tracking_conf"],
    ...
    latency_df = latency_df.melt(id_vars="Tracking Confidence",
                                  value_vars=["MediaPipe Latency (ms)", "Model Latency (ms)", "Total Latency (ms)"],
                                  var_name="Component", value_name="Latency (ms)")

    plt.title("Latency Breakdown by Tracking Confidence")
    sns.barplot(data=latency_df, x="Tracking Confidence", y="Latency (ms)", hue="Component", ci=None)
    plt.xlabel("Tracking Confidence")
    plt.ylabel("Average Latency (ms)")
    plt.legend(title="Latency Component")
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "latency_breakdown_by_track_conf.png"))

    # === PLOT 2: Per-Class and Overall Accuracy per Tracking Confidence ===
    plt.figure(figsize=(12, 6))
    acc_df = pd.DataFrame(class_acc)
    acc_df["Tracking Confidence"] = acc_df["value"]

    sns.barplot(data=acc_df, x="class", y="accuracy", hue="Tracking Confidence")
    plt.title("Gesture Class Accuracy Across Tracking Confidence")
    plt.xticks(rotation=45)
    plt.ylabel("Accuracy")
    plt.xlabel("Gesture Class (including overall)")
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "class_accuracy_by_track_conf.png"))

    # === PLOT 3: Total Accuracy vs Total Latency ===
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=df, x="total_latency", y="overall_acc", hue="tracking_conf", s=150)
    plt.title("Overall Accuracy vs Total Latency")
    plt.xlabel("Total Latency (ms)")
    plt.ylabel("Average Accuracy (L+R Hands)")
    plt.legend(title="Tracking Confidence")
    plt.tight_layout()
