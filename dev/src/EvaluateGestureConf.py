...
def run_experiments():
    ...
    for gconf in GESTURE_CONF_THRESHOLDS:
        results.append(evaluate(VIDEO_PATH, LEFT_LABEL_CSV, RIGHT_LABEL_CSV,
                                ...
                                16, MPH_TRACKING_CONFIDENCE, gconf, "gesture_conf", gconf))
    ...
    # Save data
    df.to_csv(os.path.join(RESULTS_DIR, "gesture_conf_full_results.csv"), index=False)
    acc_df = pd.DataFrame(class_acc)
    acc_df.to_csv(os.path.join(RESULTS_DIR, "gesture_conf_per_class_accuracy.csv"), index=False)


    # Insert plots here
    sns.set(style="whitegrid")
    
    # === PLOT 1: Latency Breakdown per Gesture Confidence ===
    plt.figure(figsize=(10, 6))
    latency_df = pd.DataFrame({
        "Gesture Confidence": df["gesture_conf"],
    ...
    latency_df = latency_df.melt(id_vars="Gesture Confidence",
                                  value_vars=["MediaPipe Latency (ms)", "Model Latency (ms)", "Total Latency (ms)"],
                                  var_name="Component", value_name="Latency (ms)")

    plt.title("Latency Breakdown by Gesture Confidence")
    sns.barplot(data=latency_df, x="Gesture Confidence", y="Latency (ms)", hue="Component", ci=None)
    plt.xlabel("Gesture Confidence")
    plt.ylabel("Average Latency (ms)")
    plt.legend(title="Latency Component")
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "latency_breakdown_by_gesture_conf.png"))

    # === PLOT 2: Per-Class and Overall Accuracy per Gesture Confidence ===
    plt.figure(figsize=(12, 6))
    acc_df = pd.DataFrame(class_acc)
    acc_df["Gesture Confidence"] = acc_df["value"]

    sns.barplot(data=acc_df, x="class", y="accuracy", hue="Gesture Confidence")
    plt.title("Gesture Class Accuracy Across Gesture Confidence")
    plt.xticks(rotation=45)
    plt.ylabel("Accuracy")
    plt.xlabel("Gesture Class (including overall)")
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, "class_accuracy_by_gesture_conf.png"))

    # === PLOT 3: Total Accuracy vs Total Latency ===
    plt.figure(figsize=(8, 6))
    sns.scatterplot(data=df, x="total_latency", y="overall_acc", hue="gesture_conf", s=150)
    plt.title("Overall Accuracy vs Total Latency")
    plt.xlabel("Total Latency (ms)")
    plt.ylabel("Average Accuracy (L+R Hands)")
    plt.legend(title="Gesture Confidence")
    plt.tight_layout()
